...
class market_collector:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            # Writer connections with WAL mode
            self.__markets_db = sqlite3.connect(markets_db_path, check_same_thread=False)
            self.__markets_db.execute("PRAGMA journal_mode=WAL;")
            self.__markets_db.execute("PRAGMA synchronous=NORMAL;")

            self.__attributes_db = sqlite3.connect(attributes_db_path, check_same_thread=False)
            self.__attributes_db.execute("PRAGMA journal_mode=WAL;")
            self.__attributes_db.execute("PRAGMA synchronous=NORMAL;")
    ...
    async def __query_markets(self)->bool:
        ...
        if new_markets < self.__batch_size:
            self.__log(f"market_collector {new_markets} reseting offset from {self.__market_offset} to {self.__reset_offset}", "INFO")
